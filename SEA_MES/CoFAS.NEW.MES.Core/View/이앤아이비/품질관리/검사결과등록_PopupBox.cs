...
{
    public partial class 검사결과등록_PopupBox : Form
    {
        #region ○ 폼 이동
        ...
        public 검사결과등록_PopupBox(string ID 
            , string userEntity
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                _MenuSettingEntity.BASE_TABLE = "PRODUCTION_INSPECTION_DETAIL";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                string str = $@"SELECT 
	                                   D.ID						AS 'ID'
									  ,D.STOCK_MST_ID			AS 'D.STOCK_MST_ID'
									  ,C.INSPECTION_ID			AS 'C.INSPECTION_ID'
									  ,A.STANDARD_ID			AS 'D.STANDARD_ID'
                                      ,A.STANDARD_VALUE			AS 'A.STANDARD_VALUE'
                                      ,D.VALUE				    AS 'D.VALUE'
                                      ,D.COMMENT				AS 'D.COMMENT'
                                      ,A.TYPE				    AS 'A.TYPE'								
                                      ,D.USE_YN					AS 'A.USE_YN'					
                                      ,D.REG_USER				AS 'A.REG_USER'				
                                      ,D.REG_DATE				AS 'A.REG_DATE'				
                                      ,D.UP_USER				AS 'A.UP_USER'				
                                      ,D.UP_DATE				AS 'A.UP_DATE'				
                                      ...
                                      LEFT  JOIN PRODUCTION_INSPECTION C ON B.ID = C.STOCK_MST_ID
                                      LEFT  JOIN PRODUCTION_INSPECTION_DETAIL D ON C.ID = D.INSPECTION_ID
                                      WHERE 1=1 
        ...
        private void MainSave_InputData()
        {
            try
            {
                ...
                //}
                fpMain.Focus();
                bool _Error = new CoreBusiness().BaseForm1_A10(_MenuSettingEntity,fpMain, "STOCK_INSPECTION_STANDARD");
                if (!_Error)
                {
