...
{
    public partial class 치수등록_PopupBox : Form
    {
        #region ○ 폼 이동
        ...
        public string _INSPECTION_STOCK_MST_ID = string.Empty;
        ...
        public 치수등록_PopupBox(string ID
            , string userEntity
            , string INSPECTION_STOCK_MST_ID)
        {
            _UserAccount = userEntity;

            _STOCK_ID = ID;
            _INSPECTION_STOCK_MST_ID = INSPECTION_STOCK_MST_ID;
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                _MenuSettingEntity.BASE_TABLE = "INSPECTION_STOCK_DETAIL";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                fpMain.Sheets[0].Columns["D.SPEC_IMAGE"].Visible = true;
                fpMain.Sheets[0].Columns["D.SPEC_IMAGE2"].Visible = true;

                string str = $@"SELECT 
	                                   A.ID							AS 'C.STOCK_MST_SPEC_ID'
									  ...
									  ,ISNULL(C.ID,0)				AS 'ID'
									  ,B.ID	                        AS 'C.INSPECTION_STOCK_MST_ID'
									  ,C.VALUE						AS 'C.VALUE'
	                                  ...
	                                  ,D.TYPE						AS 'D.TYPE'
                                      ,D.SPEC_IMAGE                 AS 'D.SPEC_IMAGE'
                                      ,D.SPEC_IMAGE2                AS 'D.SPEC_IMAGE2'
                                      FROM STOCK_MST_SPEC A
                                      INNER JOIN INSPECTION_STOCK_MST B ON A.STOCK_MST_ID = B.STOCK_MST_ID
									  LEFT JOIN INSPECTION_STOCK_DETAIL C ON A.ID = C.STOCK_MST_SPEC_ID
									  INNER JOIN STOCK_MST D ON A.STOCK_MST_ID = D.ID
                                      WHERE 1=1 
                                      AND A.STOCK_MST_ID = {_STOCK_ID}
                                      AND A.USE_YN = 'Y'
                                      AND B.ID = {_INSPECTION_STOCK_MST_ID}";
                ...
                if (_DataTable.Rows.Count > 1)
                {
                    ...
                    object image = fpMain.Sheets[0].GetValue(0, "D.SPEC_IMAGE");
                    object image2 = fpMain.Sheets[0].GetValue(0, "D.SPEC_IMAGE2");

                    fpMain.Sheets[0].Columns["D.SPEC_IMAGE"].Visible = false;
                    fpMain.Sheets[0].Columns["D.SPEC_IMAGE2"].Visible = false;

                    pictureBox1.Image = image as Bitmap;
                    pictureBox2.Image = image2 as Bitmap;
        ...
        private void MainSave_InputData()
        {
            try
            {
                ...
                //}
                fpMain.Focus();
                bool _Error = new CoreBusiness().BaseForm1_A10(_MenuSettingEntity, fpMain, "INSPECTION_STOCK_DETAIL");
                if (!_Error)
                {
