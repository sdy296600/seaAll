                ...
                switch (machine_no)
                {
                    case "13호기":
                        ...
                        sql2 = $@"SELECT 
                                ...
                                COALESCE(MAX(CASE WHEN Category = '{list[0]}' THEN Vl END), 0) AS V1,
                                COALESCE(MAX(CASE WHEN Category = '{list[1]}' THEN Vl END), 0) AS V2,
                                COALESCE(MAX(CASE WHEN Category = '{list[2]}' THEN Vl END), 0) AS V3,
                                COALESCE(MAX(CASE WHEN Category = '{list[3]}' THEN Vl END), 0) AS V4,
                                COALESCE(MAX(CASE WHEN Category = '{list[4]}' THEN Vl END), 0) AS ACCELERATION_POS,
                                COALESCE(MAX(CASE WHEN Category = '{list[5]}' THEN Vl END), 0) AS DECELERATION_POS,
                                COALESCE(MAX(CASE WHEN Category = '{list[6]}_Ruled' THEN Vl END), 0) AS METAL_PRESSURE,
                                COALESCE(MAX(CASE WHEN Category = '{list[7]}' THEN Vl END), 0) AS SWAP_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[8]}' THEN Vl END), 0) AS BISKIT_THICKNESS,
                                COALESCE(MAX(CASE WHEN Category = '{list[9]}' THEN Vl END), 0) AS PHYSICAL_STRENGTH_PER,
                                COALESCE(MAX(CASE WHEN Category = '{list[10]}' THEN Vl END), 0) AS PHYSICAL_STRENGTH_MN,
                                COALESCE(MAX(CASE WHEN Category = '{list[11]}' THEN Vl END), 0) AS CYCLE_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[12]}' THEN Vl END), 0) AS TYPE_WEIGHT_ENTRY_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[13]}' THEN Vl END), 0) AS BATH_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[14]}' THEN Vl END), 0) AS FORWARD_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[15]}' THEN Vl END), 0) AS FREEZING_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[16]}' THEN Vl END), 0) AS TYPE_WEIGHT_BACK_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[17]}' THEN Vl END), 0) AS EXTRUSION_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[18]}' THEN Vl END), 0) AS EXTRACTION_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[19]}' THEN Vl END), 0) AS SPRAY_TIME,
                                COALESCE(MAX(CASE WHEN Category = '{list[20]}' THEN Vl END), 0) AS CAVITY_CORE,
                                COALESCE(MAX(CASE WHEN Category = '{list[21]}' THEN Vl END), 0) AS A_POLLUTION_DEGREE,
                                COALESCE(MAX(CASE WHEN Category = '{list[22]}' THEN Vl END), 0) AS B_POLLUTION_DEGREE,
                                COALESCE(MAX(CASE WHEN Category = '{list[23]}' THEN Vl END), 0) AS VACUUM,
                                Tm AS TIMESTAMP
                            FROM 
                            ";
                        break;
