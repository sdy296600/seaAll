...
SELECT *, MST.[주조1(기본)]*MATERIAL_M_COST/100 AS '재료관리비'  , OUT_M_COST AS '외주관리비' ,MST.공정비 * GENERAL_M_COST / 100 AS  '일반관리비' FROM 
...
CONVERT(DECIMAL(18,2),INDIRECT_EXPENSE_RATIO)/100))  ,0)))
 AS '공정비'
	...
	, FORMAT((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) * 0.85)* ISNULL(F.cavity,0),'0.00')  AS '성능가동률_BEST'
    ...
    , FORMAT((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) * 0.85)* ISNULL(F.cavity,'0') * 1,'0') AS '종합가동률_BEST'

	, ((16900 /((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0'))) *1) *J.QTY_COMPLETE  AS '직접노무비_BEST'
	, ((16900 /((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0'))) *1*0.81) *J.QTY_COMPLETE  AS '간접노무비_BEST'
	, (757100998 /12/22/260/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1)) *J.QTY_COMPLETE AS '설비감상비_BEST'
	, ((((((109 *980000)*5.5)/ 40) /22)/260)/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1)) *J.QTY_COMPLETE AS '건물감상비_BEST'
	, (((757100998 /12/22/260/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1)) + ((((((109 *980000)*5.5)/ 40) /22)/260)/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1))) * 0.049) *J.QTY_COMPLETE AS '수선비_BEST'
	, (CONVERT(DECIMAL(18,2),sum(ISNULL((CONVERT(DECIMAL(18,2),ISNULL(ELECTRICAL_ENERGY,0)) * convert(int,(H.[EQUIPMENT_POWER_RATIO])) ),0)))) AS '전체전력비_BEST'
	, (((757100998 /12/22/260/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1))+((((((109 *980000)*5.5)/ 40) /22)/260)/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1))+(((757100998 /12/22/260/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1)) + ((((((109 *980000)*5.5)/ 40) /22)/260)/ ((3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME)) *0.85)* ISNULL(F.cavity,'0') *1))) * 0.049)+(CONVERT(DECIMAL(18,2),sum(ISNULL((CONVERT(DECIMAL(18,2),ISNULL(ELECTRICAL_ENERGY,0)) * convert(int,(H.[EQUIPMENT_POWER_RATIO])) ),0))))/J.QTY_COMPLETE) *0.206) *J.QTY_COMPLETE AS '간접경비_BEST'
	--,(3600/AVG(CONVERT(DECIMAL(18,2),A.CYCLE_TIME))  * 0.85) AS CT1
