...
(
    SELECT
        ...
        (SUM(CONVERT(DECIMAL(18,2), ISNULL(ELECTRICAL_ENERGY, 0) * CONVERT(DECIMAL(18,2), H.EQUIPMENT_POWER_RATIO))))　/ (SELECT SUM(QTY_COMPLETE) FROM WORK_PERFORMANCE WHERE RESOURCE_NO = A.RESOURCE_NO GROUP BY RESOURCE_NO)AS D_VALUE
...
ORDER BY 순서;";
                    ...
                    for (int i = 0; i < _DataTable.Rows.Count; i++)
                    {
                        ...
                        int rowIndex_간접노무비 = _DataTable.AsEnumerable().ToList().FindIndex(row => row["순서"].ToString() == "14");
                        ...
                        for (int num = 1; num <= 12; num++)
                        {
                            ...
                            if (rowIndex_건물감상비 != -1)  // 해당 row가 존재하면
                                ...
                                sheet.Cells[16, num + 4].SetValueFromText(_DataTable.Rows[rowIndex_건물감상비][$"{num}월"].ToString()); //건물감상비
...
";
                    ...
                    for (int i = 0; i < _DataTable.Rows.Count; i++)
                    {
                        ...
                        sheet.Cells[23, 3].SetValueFromText(pfpSpread.Sheets[0].GetValue(e.Row, "재료관리비").ToString()); //재료관리비
                        sheet.Cells[24, 3].SetValueFromText(pfpSpread.Sheets[0].GetValue(e.Row, "외주관리비").ToString()); //외주관리비
                        sheet.Cells[25, 3].SetValueFromText(pfpSpread.Sheets[0].GetValue(e.Row, "일반관리비").ToString()); //일반관리비
