...
{
    public partial class Order_Scheduler : System.Windows.Forms.UserControl
    {
        ...
        public Order_Scheduler(UserEntity pUserEntity)
        {
        ...
        public void Day_Set()
        {
            try
            {
                ...
                DataTable _DataTable = new CalendarBusiness().Calendar_Order_Info(_Main_time.ToString("yyyy-MM"));
                ...
                while (time.Month == Month)
                {
                    ...
                    DateTime start_date = DateTime.Parse(time.ToString("yyyy-MM-dd"));

                    //start_date = start_date.AddYears(time.Year);
                    //start_date = start_date.AddMonths(time.Month);
                    //start_date = start_date.AddDays(time.Day);
                   
                    //DateTime end_date = DateTime.Parse(time.ToString("yyyy-MM-dd")).AddDays(1);
                    //end_date = end_date.AddYears(time.Year);
                    //end_date = end_date.AddMonths(time.Month);
                    //end_date = end_date.AddDays(time.Day+1);

                    DataRow[] datas = _DataTable.Select($"start_date = '{start_date.ToString("yyyy-MM-dd")}'");
                    Sub_Scheduler lbl = new Sub_Scheduler(datas,_pUserEntity,false);
