...
namespace CoFAS.NEW.MES.DPS
{
    public class CoFAS_Log
    {
        ...
        private string strLastDelDate;              //로그파일 마지막으로 지운날짜..    tes
        ...
        public bool WLog(String StrMsg)
        {
            ...
            string mon = dt.Month.ToString().PadLeft(2, '0');
        ...
        public bool WLog_Hour(String StrMsg)
        {
            DateTime dt = DateTime.Now;
            string mon = dt.Month.ToString().PadLeft(2, '0');
            string day = dt.Day.ToString().PadLeft(2, '0');
            string hour = dt.Hour.ToString().PadLeft(2, '0');
            string FileName = string.Format(@"{0} {1}-{2}-{3}-{4}.txt", StrFileName, dt.Year, mon, day, hour);
        ...
        /// <param name="FileName">파일명(폴더명을 쓰지 말것)</param>
        /// <returns>성공여부</returns>
        public bool WLog_Hour(string strMsg, string FileName)
        {
            this.LogFileClear();
            ...
            lock (this)
            {
                try
                {
                    ...
                    string day = dt.Day.ToString().PadLeft(2, '0');
                    string hour = dt.Hour.ToString().PadLeft(2, '0');
                    FileName = string.Format(@"{0} {1}-{2}-{3}-{4}-{5}.txt", StrFileName, dt.Year, mon, day, hour, FileName);
                    string Msg = string.Format("[{0}:{1}:{2}] {3}\r\n", dt.ToString("HH"), dt.ToString("mm"), dt.ToString("ss"), strMsg);
                    byte[] bytMsg = Encoding.Default.GetBytes(Msg);
